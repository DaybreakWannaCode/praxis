...
def run(cfg, manifest, output):
    ...
    record=dict(status='running',source_config=cfg,scope='greedy throughput only, warm parent, no test data or training',
    ...
    summary=dict(count=len(rows),startup_seconds=startup,decode_seconds=seconds,
                 ...
                 scope=record['scope'],warning='One warm parent and availability-limited panel; future checkpoints may have different response lengths. Not vLLM throughput.')
