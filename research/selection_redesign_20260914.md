...
Retain the original one-stage decision-training entry point and composite text reward. The existing four-prompt engineering data must not be called a meaningful training corpus. Audit the actual released text inventory, scene identities, images and overlap before freezing the run. Use a single declared pass over the eligible training corpus, with an explicit numeric optimizer-step count computed from that inventory and the original batch/rollout settings. Do not substitute a guessed step count. A runtime forecast must precede launch; if unaffordable, revise and record the budget before any endpoint outcomes are inspected.
