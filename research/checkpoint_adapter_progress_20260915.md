...
## Concrete validation/quota binding

`checkpoint_binding.install_checkpointing` now supplies the adapter with actual Torch serialization, the semantic checkpoint validator and whole-volume quota accounting. It verifies the pinned reference-schema SHA before installation, restricts topology to one rank and snapshots explicit optimizer/scheduler/LR expectations for each permitted save step. Unknown save steps fail before the worker writes. Driver steps are not silently assumed to equal optimizer or scheduler counters, particularly after warm-parent restoration.

Seven local tests passed across binding and adapter. The new binding test exercises actual Torch dataloader serialization and publication with a small actor fixture, confirms hash rejection before installation, checks expectations remain sealed against caller mutation and rejects an unplanned next save while preserving the previous published checkpoint. Semantic validation is mocked in this binding test; its real-checkpoint coverage remains the separately recorded structural audit. This is an explicit entrypoint ready for a newly owned trainer, not an installed change to a live or historical run.

Read-only inspection of the original `FSDPCheckpointManager.save_checkpoint` found model/optimizer/extra-state writes, barriers and HuggingFace configuration/tokenizer writes under the supplied path; that method does not retire old directories. The adapter avoids the driver's prior delete-before-save path. Full-worker publication, interruption behavior across Ray/FSDP and subsequent resume still require the bounded capacity-dependent integration run. No GPU job, historical deletion or storage allocation occurred in this stage.
