...
def main():
    ...
    env = os.environ.copy()
    for key in ('PRAXIS_FIXED_INPUT','PRAXIS_RESUME_DELTA_DIR','PRAXIS_ALIGNMENT_CAPTURE_DIR'):
        env.pop(key,None)
    ...
    (a.output/'launch.json').write_text(json.dumps(dict(command=command,start_unix=start,
        ...
        prompt_inventory=a.prompt_inventory,timeout_seconds=a.seconds),indent=2))
