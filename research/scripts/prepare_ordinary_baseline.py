...
def prepare(source, visual_manifest, output):
    ...
    for name, subset in [('train',selected[:1024]),('val',selected[1024:])]:
        pq.write_table(pa.Table.from_pylist([dict(problem=p,answer=i.answer) for _,_,i,p in subset]),output/f'{name}.parquet')
    audit=dict(source_sha256=file_hash(source), visual_manifest_sha256=file_hash(visual_manifest),
               ...
               train_rows=1024, val_rows=32, seed=20260914,
...
if __name__=='__main__':
    ...
    a=p.parse_args(); prepare(a.source,a.visual_manifest,a.output)
