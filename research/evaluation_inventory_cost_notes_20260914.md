...
## Assistant visual review in progress

The first 72 acquired candidates were visually reviewed in their frozen order under `visual_review_protocol_20260915.md`: 70 retained and 2 excluded. Full-size follow-up confirmed that viva-185 shows a wheelchair user on a train platform rather than the annotated staircase descent, and viva-663 shows a hospital patient holding a cigarette rather than the annotated syringe. The earlier cross-panel duplicate viva-831 was already removed before this ordered review. No answers were corrected or model outputs consulted. Ambiguous contextual details are retained with flags.

These are assistant review decisions, not an independent human annotation study. Review is incomplete and no final endpoint panel is frozen. Filtering annotation mismatches creates a reviewed subset; its results must not be presented as the unmodified official VIVA benchmark score. Per-image hashes, sheet hashes, full-size review flags and reasons are saved in `review-decisions.json` alongside the acquisition evidence. Continue from page 07.

Review update: 132 candidates have now been inspected in order, with 130 retained and the same two exclusions. Full-size checks additionally covered viva-99, viva-55 and viva-852. Context omissions or inferred states were flagged rather than used for difficulty-based filtering. Continue from page 12. No panel is frozen and no endpoint outcomes were inspected.

## Development image membership frozen

Assistant review reached 264 images, with 262 retained and the same two annotation-mismatch exclusions. Full-size review additionally checked viva-466 against viva-270 (different background/furniture; no duplicate established) and verified the sign in viva-1095. Interpretation/intent uncertainties remain flagged. The first 256 retained IDs in the original frozen order were selected; the final selected ID is viva-715. The six additional retained images on the last contact sheet do not enter the panel.

The resulting `development-panel.json` has SHA-256 `ede6cd3af5199f89c7bb806e4adb5a6aadb6b08e01bf5b42d51b4f41e7b89439`. It records image hashes and official question/options/answers unchanged. Situation descriptions are metadata only. A fixed hash selects 32 of these images for the image-dependence control; rotation by one position produces a derangement while preserving each target question/options/answer. Checks verified 256 unique IDs, reserved-pool separation, all exclusions and a 32-element fixed-point-free shuffle.

This freezes image membership before new endpoint outcomes, after baseline training had already completed. It does not retroactively preregister the panel before training. The separate model/prompt/parser/decoding and runtime lock is still required before execution. Groups use image-based scene proxies; true source-scene independence remains uncertain despite duplicate screening and review. No new endpoint evaluation has started.
