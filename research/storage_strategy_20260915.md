...
Next gate: validate replay-input capture in the original candidate worker and demonstrate two sequential candidates through the durable-receipt/temporary-cleanup lifecycle. The real-model scorer has passed; see compact_choice_result_20260915.md. Then revise the execution estimate. The full matrix remains unlaunched.
...
## Completed second lifecycle and current boundary

Both candidate lifecycles are now complete and locally audited. Together they released 13,177,864,544 bytes of managed temporary files and retained about 34.3 MB of durable evidence. The second lifecycle retained 17,158,973 bytes. This supersedes the earlier statement that the second candidate is merely launched. Automatic temporary export cleanup is validated for these two original-worker cases; exact optimizer replay and original-trainer checkpoint rotation remain unvalidated. No historical checkpoint has been deleted. See selection_reassessment_20260915.md for the updated conditional compute comparison and the remaining archive/checkpoint gate.

## Refreshed footprint and six-run retention envelope

Read-only measurement on September 15 found 217,404,540,416 allocated bytes under `/workspace` (217.405 decimal GB), including 200.304 GB under `/workspace/praxis`, 9.229 GB of environments and 7.530 GB of model cache. The runs directory reports approximately 187 GiB. No GPU compute processes appeared in the accompanying query. This is a snapshot, not a billing-quota check: `df` reports the shared provider filesystem's hundreds of terabytes and must not be treated as this volume's allowance.

For the smaller two-selector, three-seed proposal, retaining all old history plus six final FP32 models, two active full-checkpoint slots, bounded scratch/audit exports, new small data and reserve projects **481.585 GB**. Reproduce with `python3 research/scripts/plan_storage_retention.py --existing-bytes 217404540416 --runs 6`. The nine-run variant projects 530.395 GB. Both calculations deliberately count final-model copies alongside active recovery slots; neither assumes an unverified archive or future compression gain.

Thus 550 GB is a conservative keep-all-history option for the reduced study, not a requirement of alignment itself. The 232.11 GB working-set design remains possible only with a functioning, verified archive and removal of archived historical copies. Current 250 GB cannot accommodate even one additional 41.28 GB full recovery checkpoint alongside this footprint and reserve. No storage was purchased or historical files removed during this reassessment. The existing request for compute/storage approval remains pending.

The real original-worker restore audit has now passed for an existing checkpoint, including model, optimizer, scheduler and RNG equality. This supersedes earlier uncertainty about loading that checkpoint; it does not validate the new save-publication adapter or replay a subsequent optimizer update. Full-size publication remains the next capacity-dependent integration check.
